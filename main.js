...
check.onclick = function() {
  ...
  let intEvent = document.getElementById('input').value;
  let result = (Math.abs(checkEvent(intEvent, JOURNAL)) * 100).toFixed(2) + "% is the Problem";
...
function checkEvent(event, journal) {
  ...
  for (let i = 0; i < journal.length; i++) {
    ...
    let isExist = journal[i];
    if (isExist.events.includes(event)) {
      counter++;
    }
  }
  ...
  if (counter === 0) {
    ...
    return calcPercentage(event);
  }
}

function calcPercentage(event) {

  function fancEve(event, journal) {
    ...
    for (let i = 0; i < journal.length; i++) {
      ...
      let dayData = journal[i];
      let index = 0;

      if (dayData.events.includes(event)) {
        index += 1;
      }
      if (dayData.squirrel) {
        index += 2;
